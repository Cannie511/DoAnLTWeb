...
if (isset($_GET["search"]) && $_GET["search"] !== "") {
    ...
    try {
        ...
        $query = "SELECT * FROM bai_hat JOIN trinhbay JOIN nghesi JOIN quocgia ON bai_hat.Ma_Bai_Hat = trinhbay.Ma_Bai_Hat 
        and  trinhbay.Ma_NS = nghesi.Ma_NS and bai_hat.Ma_Quoc_GIa = quocgia.Ma_Quoc_GIa WHERE LOWER(TRIM(Ten_Bai_Hat)) LIKE :timKiem OR LOWER(TRIM(Ten_Ca_Si)) LIKE :timKiem";
        ...
        if (count($result) > 0) {
            foreach ($result as $key => $music) {
                ...
                echo "</td>";
                echo "<td>";
                print_r($result[$key]['Ten_Quoc_Gia']);
                echo "</td>";
                echo "<td>";
