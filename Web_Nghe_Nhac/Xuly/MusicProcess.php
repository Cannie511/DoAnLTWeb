...
{
    include("DB/ketnoi.php");
    $sql = "SELECT * FROM bai_hat JOIN trinhbay JOIN nghesi JOIN the_loai ON bai_hat.Ma_Bai_Hat = trinhbay.Ma_Bai_Hat 
    and  trinhbay.Ma_NS = nghesi.Ma_NS and bai_hat.Ma_The_Loai = the_loai.Ma_The_Loai ORDER BY Ngay_Phat_Hanh DESC";
    $stm = $conn->prepare($sql);
    $stm->execute();
    $data = $stm->fetchAll(PDO::FETCH_ASSOC);
    $no = 1;
    foreach ($data as $key => $music) {
        ...
        echo "<td><ion-icon name='play'  
        data-music-link='" . $data[$key]['path'] .
            "'data-img-link ='" . $data[$key]['Anh_Bia'] . "' data-title-link = '" . $data[$key]['Ten_Bai_Hat'] . 
            "' data-singer-link='" . $data[$key]['Ten_Ca_Si'] .
            "' data-the-loai='" . $data[$key]['Ten'] .  
            "' onclick='playMusic(this, " . $data[$key]['Ma_Bai_Hat'] . ")'>
        </ion-icon></td>";
        echo "</td>";
        echo "<td>";
        print_r("#" . $no);
